...
#     <overlay>              overlay dir under apps/registries/overlays/  (e.g. staging)
...
#
#  PREREQUISITE: apps/registries must exist in oan-kustomize. It does not yet, so
#  this script fails fast with a clear message until the overlay is added.
# =============================================================================
set -euo pipefail
...
OVERLAY_PATH="apps/registries/overlays/${OVERLAY}"
...
if [ ! -d "$WORK/repo/${OVERLAY_PATH}" ]; then
  echo "!! ${OVERLAY_PATH} not found in oan-kustomize."
  echo "!! Add the registries app (apps/registries/base + overlays/${OVERLAY}) before enabling the staging GitOps deploy."
  exit 1
fi
...
git commit -m "ci(registries): ${OVERLAY} image -> ${NEW_REF##*/}"
