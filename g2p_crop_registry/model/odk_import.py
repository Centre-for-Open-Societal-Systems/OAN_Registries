...
class OdkImport(models.Model):
            ...
            try:
                with self.env.cr.savepoint():
                    ...
                    if current_instance_id:
                        # Check if this ODK instance was already imported as a crop registry
                        is_already_imported = False

                        # First check planning lines
                        if self.env['g2p.annual.line'].sudo().search([('sync_id', '=', current_instance_id)], limit=1):
                            is_already_imported = True
                        # Then check production lines (if it was a cultivation/harvesting form)
                        elif self.env['g2p.crop.production'].sudo().search([('sync_id', '=', current_instance_id)], limit=1):
                            is_already_imported = True

                        if is_already_imported:
                            _logger.info("Skipping duplicate ODK submission with instance ID: %s", current_instance_id)
                            # Do not increment partner_count or skipped_count here,
                            # we want it to be silently ignored so the UI shows "No new form records were submitted"
                            continue
                                    ...
                                    for cmd in mapped_json[line_field]:
                                        if isinstance(cmd, (list, tuple)) and len(cmd) == 3 and cmd[0] == 0:
                                            ...
                                            if not isinstance(line_vals, dict):
                                                continue
                                            land_ref = line_vals.get('temporary_land_id') or line_vals.get('land_id') or line_vals.get('land_info_id')
                                            if not land_ref:
                                                continue
                                            ...
                                            if matched_land:
                                                ...
                                                if isinstance(land_ref, str) and str(land_ref).strip() and str(land_ref).strip() != 'False':
                                                    ...
                                                    if not land_rec:
                                                        ...
                                                        if land_rec and land_rec.partner_id and land_rec.partner_id.id != partner.id:
                                                            fayda_disp = land_rec.partner_id.fayda_id or land_rec.partner_id.name or "Unknown"
                                                            raise ValidationError(f"Import Rejected: Land ID '{l_id_str}' is already registered to a different farmer ({fayda_disp}).")
                                                elif isinstance(land_ref, int):
                                                    land_rec = self.env['g2p.land.information'].sudo().search([('id', '=', land_ref)], limit=1)
                                                    if land_rec and land_rec.partner_id and land_rec.partner_id.id != partner.id:
                                                        fayda_disp = land_rec.partner_id.fayda_id or land_rec.partner_id.name or "Unknown"
                                                        raise ValidationError(f"Import Rejected: Land ID (ID: {land_ref}) is already registered to a different farmer ({fayda_disp}).")
                                ...
                                for cmd in mapped_json[line_field]:
                                    if isinstance(cmd, (list, tuple)) and len(cmd) == 3 and cmd[0] == 0:
                                        ...
                                        if isinstance(line_vals, dict):
                                            # Prefer temporary_land_id or land_id for the string if available
                                            land_ref = line_vals.get('temporary_land_id') or line_vals.get('land_id') or line_vals.get('land_info_id')
                                            if not land_ref:
                                                continue
                                            ...
                                            # Collision check before creating new land for the temp farmer
                                            if land_id_val and land_id_val not in ('False', 'None', ''):
                                                domain_any = []
                                                if land_id_val.isdigit():
                                                    domain_any.append(('land_id', 'in', [land_id_val, f"Plot {new_partner.id}-{land_id_val}"]))
                                                else:
                                                    domain_any.append(('land_id', '=', land_id_val))
                                                conflict_land = self.env['g2p.land.information'].sudo().search(domain_any, limit=1)
                                                if conflict_land and conflict_land.partner_id and conflict_land.partner_id.id != new_partner.id:
                                                    fayda_disp = conflict_land.partner_id.fayda_id or conflict_land.partner_id.name or "Unknown"
                                                    raise ValidationError(f"Import Rejected: Land ID '{land_id_val}' is already registered to a different farmer ({fayda_disp}).")
                            ...
                            for cmd in mapped_json[line_field]:
                                if isinstance(cmd, (list, tuple)) and len(cmd) == 3 and isinstance(cmd[2], dict):
                                    ...
                                    if l_id:
                                        ...
                                        if l_int:
                                            crop_a = 0.0
                                            for a_key in ['actual_crop_area', 'area_sown', 'crop_planned_area']:
                                                if l_vals.get(a_key):
                                                    try:
                    ...
                    if existing:
                        if self.target_registry in ['g2p.crop.registry.planning']:
                            _logger.info("Record already exists. Skipping updates for planning/cultivation. id=%s", existing.id)
                            continue
                        ...
                        if self.target_registry in ['g2p.crop.registry.sowing_harvesting', 'g2p.crop.registry.harvesting']:
                            raise Exception("Import Rejected: Sowing/Harvesting data can only be imported for existing Planning or Cultivation records.")
                ...
                _logger.error("Failed to import ODK record %s: %s", member.get('__id', 'UNKNOWN'), e, exc_info=True)
                skipped_count += 1
                error_list = data.setdefault('error_messages', [])
                # Store a clean string for the UI message
                error_list.append(f"Record {member.get('__id', 'UNKNOWN')}: {str(e)}")
                continue
        if skipped_count > 0:
            # Inject error messages into the partner_count string so it shows up in the UI popup
            # since odk_client.py hardcodes the message format as f"ODK form {partner_count} records..."
            error_details = ", ".join(error_list)
            display_count = f"{partner_count} (Skipped: {skipped_count} | Reasons: {error_details})"
            data.update({'partner_count': display_count})
        else:
            data.update({'partner_count': partner_count})

        data.update({'skipped_count': skipped_count})

        if skipped_count > 0 or partner_count > 0:
            data.update({'form_updated': True})
    ...
    def _map_aliases_recursive(self, model_name, vals):
        ...
        aliases = {
            'land_info_id': ['land_info_id', 'temporary_land_id', 'land_id'],
    ...
    def _resolve_m2o_recursive(self, val, partner_id=False):
        if isinstance(val, dict):
            ...
            land_val = val.get('land_info_id') or val.get('temporary_land_id') or val.get('land_id')
            if land_val:
                    land_val_str = str(land_val).strip()
                    domain_any = []
                    if land_val_str.isdigit() and current_partner_id:
                        domain_any = ['|', ('land_id', '=', land_val_str), ('land_id', '=', f"Plot {current_partner_id}-{land_val_str}")]
                    else:
                        domain_any = [('land_id', '=', land_val_str)]
                    land_rec = self.env['g2p.land.information'].sudo().search(domain_any, limit=1)
                    if land_rec and land_rec.partner_id and land_rec.partner_id.id != current_partner_id:
                        fayda_disp = land_rec.partner_id.fayda_id or land_rec.partner_id.name or "Unknown"
                        raise ValidationError(f"Import Rejected: Land ID '{land_val_str}' is already registered to a different farmer ({fayda_disp}).")
                    ...
                    if not land_rec:
                        try:
                            ...
                            if -2147483648 <= land_db_id <= 2147483647:
                                ...
                                if land_rec and land_rec.partner_id and land_rec.partner_id.id != current_partner_id:
                                    fayda_disp = land_rec.partner_id.fayda_id or land_rec.partner_id.name or "Unknown"
                                    raise ValidationError(f"Import Rejected: Land ID (ID: {land_db_id}) is already registered to a different farmer ({fayda_disp}).")
                        except (TypeError, ValueError):
                            pass
