...
class G2PCrop(models.Model):
    ...
    # Actual Annual fields (for Cultivation Details comparison)


    # Mismatch computed fields for annual
    ...
    def get_dashboard_stats(self, filters=None):
        ...
        # Calculate totals
        total_planned_area = sum(records.mapped('annual_line_ids.crop_planned_area')) + sum(records.mapped('perennial_line_ids.crop_planned_area')) + sum(records.mapped('biennial_line_ids.crop_planned_area'))
        total_expected_yield = sum(records.mapped('annual_line_ids.crop_expected')) + sum(records.mapped('perennial_line_ids.crop_expected')) + sum(records.mapped('biennial_line_ids.crop_expected'))
        
        total_actual_area = sum(records.mapped('actual_annual_line_ids.actual_crop_area')) + sum(records.mapped('actual_perennial_line_ids.actual_crop_area')) + sum(records.mapped('actual_biennial_line_ids.actual_crop_area'))
        total_actual_yield = sum(records.mapped('actual_annual_line_ids.actual_yield')) + sum(records.mapped('actual_perennial_line_ids.actual_yield')) + sum(records.mapped('actual_biennial_line_ids.actual_yield'))
        ...
        for rec in records:
            for line in rec.annual_line_ids:
                actual_lines = rec.actual_annual_line_ids.filtered(lambda l: l.crop_name_id == line.crop_name_id and l.season_id == line.season_id)
...
class G2PAnnualLine(models.Model):
    ...
    def _compute_actual_crop_category(self):
        for rec in self:
            if rec.actual_crop_name_id:
                rec.actual_crop_category_id = rec.actual_crop_name_id.category_id.id
            else:
                rec.actual_crop_category_id = False
