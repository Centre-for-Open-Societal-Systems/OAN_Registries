...
class TestPerennialCrop(TransactionCase):
    ...
    def test_perennial_crop_plan_creation(self):
        plan = self.env['g2p.perennial.line'].create({
    ...
    def test_perennial_crop_actual_creation(self):
        actual = self.env['g2p.perennial.actual.line'].create({
            'crop_registry_id': self.crop_registry.id,
            'crop_name_id': self.crop.id,
            'season_id': self.season.id,
        })
    ...
    def test_collected_gc_outside_season_raises(self):
        with self.assertRaises(ValidationError):
            self.env['g2p.perennial.line'].create({
                'crop_registry_id': self.crop_registry.id,
                'crop_name_id': self.crop.id,
                'season_id': self.season.id,
                'start_gc': self.season.start_gc,
    ...
    def test_onchange_actual_yield(self):
        ...
        actual = self.env['g2p.perennial.actual.line'].new({
            'crop_registry_id': self.crop_registry.id,
            'crop_name_id': self.crop.id,
            'season_id': self.season.id,
            'sync_id': sync_id,
    ...
    def test_onchange_crop_planned_area(self):
        ...
        plan = self.env['g2p.perennial.line'].new({
        ...
        registry.perennial_line_ids |= plan
    ...
    def test_onchange_actual_crop_area(self):
        ...
        registry.actual_perennial_line_ids |= actual
    ...
    def test_onchange_land_info_id(self):
        plan = self.env['g2p.perennial.line'].new({'land_info_id': self.land_info.id})
        ...
        actual = self.env['g2p.perennial.actual.line'].new({'land_info_id': self.land_info.id})
    ...
    def test_season_onchange_and_computes(self):
        plan = self.env['g2p.perennial.line'].new({'season_id': self.season.id})
        ...
        actual = self.env['g2p.perennial.actual.line'].new({'season_id': self.season.id})
    ...
    def test_crop_category_and_onchange(self):
        plan = self.env['g2p.perennial.line'].new({'crop_name_id': self.crop.id})
        ...
        actual = self.env['g2p.perennial.actual.line'].new({'crop_name_id': self.crop.id})
    ...
    def test_onchange_collected_ec_to_gc(self):
        plan = self.env['g2p.perennial.line'].new({'collected_ec': '12-10-2017'})
        plan._onchange_collected_ec()
        self.assertIsNotNone(plan.collected_gc)

        actual = self.env['g2p.perennial.actual.line'].new({'collected_ec': '12-10-2017'})
    ...
    def test_local_and_scientific_name_propagation(self):
        ...
        self.env['g2p.perennial.line'].create({
            ...
            'season_id': self.season.id,
            'sync_id': sync_id,
            'local_name': 'Local Apple',
            'scientific_name': 'Malus'
        })

        # Trigger the sync from crop registry
        self.crop_registry._onchange_sync_perennial_lines()

        # Verify the actual line was created with the propagated names
        actual = self.env['g2p.perennial.actual.line'].search([('sync_id', '=', sync_id)])
        self.assertEqual(len(actual), 1)
        self.assertEqual(actual.local_name, 'Local Apple')
        self.assertEqual(actual.scientific_name, 'Malus')
